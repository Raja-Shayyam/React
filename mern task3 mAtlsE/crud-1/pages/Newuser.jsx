...
import { PostingData, Getone, Getpost, } from "../getAXI0S/axiousing"
...
export const Newuser = () => {
  ...
  const getingForID = async () => {
    console.log(']] ' + myuser.length);
    // const uid = await Getone(myuser.length)
    const geti = await Getpost()
    console.log('geti ', geti.data[myuser.length - 1].id);
    const rslt = await geti.data[myuser.length - 1].id
    return (rslt+1)
    // console.log('uid ',uid);
    // return uid.data.id
  }
